...
short dk2m_ferror(const char* format, ...)
{
    ...
    va_list val;
    va_start(val, format);
    vsprintf_s(errmessage, sizeof(errmessage), format, val); // Use vsprintf_s instead of vsprintf
    va_end(val);
    short result;
    ...
    if (fplen>1)
    {
      ...
      if ((chr=='\\')||(chr=='/'))
      {
        if (fplen>2)
          ...
          {
              strcpy_s(fpath + fplen, strlen(fpath) - fplen + 1, "..\\");
              return;
          }
        fplen--;
      }
...
long dk2m_fread_chunk(struct DK2M_Chunk *chunk,FILE *fp,unsigned long offset,short flags)
{
  ...
  if (fseek(fp,offset,SEEK_SET) != 0)
  {
      if (flags & DK2MFLAG_VERBOSE) {
          ...
          dk2m_ferror("%s while seeking for chunk", err_buffer); // Pass the error message to dk2m_ferror
      }
      return -1;
  }
  ...
  if (nread != SIZEOF_DK2M_Chunk_Header)
  {
      ...
      if (flags&DK2MFLAG_VERBOSE){
          char err_buffer[256]; // Buffer to store the error message
          strerror_s(err_buffer, sizeof(err_buffer), errno); // Properly call strerror_s
          dk2m_ferror("%s while reading chunk header", err_buffer); // Pass the error message to dk2m_ferror
      }
      return -1;
  }
  ...
  if (nread != chunk->dsize)
  {
      ...
      if (flags&DK2MFLAG_VERBOSE){
          char err_buffer[256]; // Buffer to store the error message
          strerror_s(err_buffer, sizeof(err_buffer), errno); // Properly call strerror_s
          dk2m_ferror("%s while reading data", err_buffer); // Pass the error message to dk2m_ferror
      }
      if (nread<chunk->dsize)
...
char *dk2m_get_lvflags_shorttext(const unsigned int lvflags)
{
    ...
    for (i=0;i<16;i++)
    {
      ...
      if (lvflags&mask)
      {
          strcpy_s(flagstext, sizeof(lvflags_shorttext) - (flagstext - lvflags_shorttext), lvflg_shortexts[i]);
...
short dk2m_read_mapfiles(struct DK2_Level *lvl,const char *name,short flags)
{
  ...
  for (i=0;i<lvl->filepaths_count;i++)
  {
      ...
      strcat_s(fname, strlen(name) + SIZEOF_DK2_FilePathStr, onlyname);
      if (strchr(onlyname,'.')==NULL)
          strcat_s(fname, strlen(name) + SIZEOF_DK2_FilePathStr, ".kwd");
